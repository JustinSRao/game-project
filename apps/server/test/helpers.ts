 ...
 * packages/director/test/helpers.ts (the canonical FakeModelClient pattern;
 * per CLAUDE.md we don't import across package test directories). Values are
 * validated through the same Zod schema the real client would use, so a
 * canned payload that doesn't satisfy the schema fails the test too.
 */
export class FakeModelClient implements ModelClient {
  ...
  async generateStructured<T>(req: StructuredRequest<T>): Promise<T> {
    ...
    if (next === undefined) {
      throw new Error(`FakeModelClient queue empty (call #${this.calls.length})`);
    }
    ...
    acts: [
      {
        ...
        beats: [
          {
            id: "beat-bell",
            summary: "The bell arrives and takes an interest in the player.",
            status: "pending",
          },
          {
            id: "beat-marlow-secret",
            summary: "Marlow reveals what they carry.",
            status: "pending",
          },
        ],
      },
