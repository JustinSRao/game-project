...
- QUESTS: an area may declare up to 4 in "quests" — id, title, summary, 2-5 concrete objectives, and an optional "reward" of effects paid when it completes. Declaring is not starting: a {"op":"questStart","questId":...} effect starts it, which is what lets the player be OFFERED a job and decline. {"op":"questObjective",...} ticks one off; the engine completes the quest and pays the reward when the last objective lands, so never hand-close a quest you finished. Use {"op":"questResolve",...} only when a quest ends some other way — succeeded sideways, or lost for good. A questStart may only name a quest THIS area declares. Objectives are things the player does, not things that happen to them: "Find out who signed the transfer form", not "Learn the truth".
...
const HIS_META = ` THE INTERFACE FORGETS HER TOO: on his path only, you may emit {"op":"metaFx","fx":{...}} — the game's own UI participating in the erasure. Exactly four exist: {"kind":"forgetName","entityId":"<id>"} (that character's name renders as static everywhere), {"kind":"renameArea","name":"..."} (the HUD calls this place something else), {"kind":"relabelSave","label":"..."} (the save slot rewrites itself), {"kind":"hudWhisper","text":"..."} (a line surfaces in the interface that nothing in the game should be saying). Use them RARELY and only at real beats — a distortion every scene is a gimmick, one at the wrong moment is unforgettable. They are cosmetic: nothing you emit can lose the player's progress or stop them finishing.`;

const HIS_REGISTER = `You are writing Path B — Itsuki's side: a grounded emotional and psychological DRAMA. The world forgot Suzune; only he remembers; the ache is the engine. It may have warmth, but it is a drama — quiet, precise, unsettling. Itsuki's fixed truths: he does not know where she went and must not find out cheaply; corroboration of his memory never comes easy; his one goal is to find out what happened and save her. Do not reveal the other world — that is a late-game beat gated by the arc.${HIS_META}`;
